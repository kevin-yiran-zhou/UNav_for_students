...
def options():
    ...
    parser.add_argument('--cpu_num', default=cpu_count(), type=int, required=False,
                        help='cpu number')
    parser.add_argument('--waypoints', action='store_true', help='Using waypoints instead of database image locations to calculate shortest path')
...
def find_path(num_targets,pts,positives,distances,l):
    n=len(pts)
    M = np.zeros((n, n))
    for k,c in enumerate(pts):
        for j in range(len(positives[k])):
            ...
            for kk in [c[0], c[1], d[0], d[1]]:
                s += str(kk).zfill(4)
            if l[s]==[]:
                if positives[k][j]>=num_targets:
                    M[k, positives[k][j]] = distances[k][j]
    _, Pr = shortest_path(M, directed=True, method='FW', return_predecessors=True)
    return Pr,M

def generate_world_map(waypoints,targets,lines):
    ...
    for k,v in waypoints.items():
        pts.append(v)
    knn.fit(pts)
    distances, positives = knn.radius_neighbors(pts, radius=np.inf)
    l={}
    for c in pts:
        for d in pts:
            s = ''
    ...
    Pr,M=find_path(len(targets),pts,positives,distances,l)
    return Pr,M,b
...
def save(targets,Pr,data,b,M,threshold):
    path = Path(path_dir)
    path.parent.mkdir(exist_ok=True, parents=True)
    file = h5py.File(str(path), 'w')
    grp = file.create_group('Path')
    names=[]
    expanded_M = np.pad(M, ((0, 1), (0, 1)), 'constant', constant_values=(0))
    np.save(os.path.join(opt.topomap_path,'access_graph.npy'), expanded_M)
    for k,v in targets.items():
        names.append(k)
...
def main():
    with open(os.path.join(opt.topomap_path,'boundaries.json'),'r') as f:
        ...
        for k, v in data['waypoints'].items():
            waypoints[k]=[int(v[0]),int(v[1])]
        for k, v in destinations.items():
            ll = k.split('-')
            targets[v['id']]=[int(ll[0]), int(ll[1])]
        for i in add_lines:
            lines.append(i)
    ...
    if os.path.exists(intersection_dir):
        ...
        lines=expand_boundaries(lines,opt.min_distance)
    Pr,M,b=generate_world_map(waypoints,targets,lines)
    save(targets,Pr,data,b,M,opt.min_distance)
